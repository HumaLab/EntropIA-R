...
- `"auto"` (default) — extraction when present, otherwise transcription (this
  mirrors the rule EntropIA's own full-text index uses).
...
```{r}
entropia_text(con) |> entropia_collect() |> dplyr::filter(is.na(text))
```
...
```{r}
ids <- entropia_assets(con) |> dplyr::filter(type == "audio") |> dplyr::pull(id)
...
Malformed JSON never fails the collect — it warns and yields `NA`, and the raw
value stays reachable via `parse = FALSE`.

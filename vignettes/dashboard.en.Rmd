---
title: "Local dashboard and frozen reports"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Local dashboard and frozen reports (English)}
...
*English version.* Spanish: `vignette("dashboard")`.

The dashboard and the Quarto report **reuse** `entropia_overview()` tables.
They do not implement a second set of metrics.
...
Always point the app at a **self-contained snapshot** (`entropia_copy()`).
A nonempty `-wal` sidecar is rejected: the dashboard is not a live viewer of
the desktop app.
...
`entropia_dashboard()` returns a `shiny.appobj`. Starting the server is
explicit:
...
folders named “Archivo” do not collapse. Entity source / model / confidence
affect entity tables only, not the item universe.
...
reader’s local SQLite file. `redact = TRUE` (default) drops `source_path` and
replaces collection / entity / topic labels with report-local names.
Aggregates can still disclose information — review before sharing.
...
- Length profiles are the first N rows in stable order, not a random sample.

Next: `vignette("administration")` for WAL, locks and the write stubs.

...
- data:
  ...
  Data to insert (data frame/tibble).
...
generating UUIDv4 primary keys for rows that do not carry one. It never
uses `INSERT OR REPLACE` (rowid reassignment breaks FTS5).
`dry_run = TRUE` (default) validates against the column contract without

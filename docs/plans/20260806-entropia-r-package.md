...
- [x] implement `entropia_analysis_dataset()` (corpus → filters → collect → class `entropia_dataset` + `entropia_prov` attributes) (R/analysis.R: `entropia_analysis_dataset()` captures filter quosures via `rlang::enquos`, applies them to the lazy corpus, arranges by `asset_id` for deterministic row order, collects via `entropia_collect`, and stamps an `entropia_prov` attribute with `name`/`schema_version`/`content_hash`/`source_path`/`filters`/`package_version`/`built_at`/`r_version`; class `c("entropia_dataset", class(data))`; `print`/`glimpse`/`as_tibble` methods registered)
- [x] implement `entropia_provenance()` / `entropia_write_provenance()` (JSON sidecar) (R/export.R: `entropia_provenance()` reads the `entropia_prov` attribute into a classed `entropia_provenance` list with a print method; `entropia_write_provenance()` serialises it to a UTF-8 JSON sidecar with `NULL`/`NA` as JSON `null` so it round-trips through `jsonlite::fromJSON`)
- [x] write tests: provenance fields correct (schema version, content hash, captured filters, package version); JSON round-trip; dataset class methods (print/glimpse/as_tibble) (tests/testthat/test-dataset.R, 49 assertions: typed dataset class + provenance fields, whole-corpus unnamed build with empty filters, multi-filter SQL push-down, two-build determinism (identical data + stable provenance, only `built_at` differs), provenance error on unstamped objects, JSON round-trip, path validation, print/glimpse/as_tibble methods, name/named-filter/bad-column/closed-connection validation)
- [x] run tests — must pass before Task 23 (full suite 1358 expectations / 0 fail / 0 warn, 1 pre-existing CRAN-gated skip in test-compat.R (snapshot) that fires when `NOT_CRAN` is unset; lint matches the established profile (cross-file `object_usage` artifact + test SCREAMING_SNAKE constants only); `devtools::check()` 0 errors / 0 warnings / 1 known baseline note — unused Imports lifecycle/stringr reserved for Tasks 26-28)
- **Acceptance:** building a dataset twice yields identical content hash and reproducible provenance.
  - Verified 2026-08-06 with R 4.5.2 on a `VACUUM INTO` copy of the 29 MB reference DB (the Task 8 live-open segfault note still applies; sqlite3 CLI made the copy): `entropia_analysis_dataset(con, asset_type == "image")` → 2428 rows; two builds share an identical content hash (`1aba317e…`), schema version, filters, package version and byte-identical data (only `built_at` differs); `entropia_write_provenance()` writes a JSON sidecar that round-trips identically through `jsonlite::fromJSON`.

### Task 23: Visualization helpers
- [x] implement `entropia_plot_coverage()`, `entropia_plot_temporal()`, `entropia_plot_entities()` (Suggests ggplot2, `requireNamespace` guard, return `ggplot`) (R/plot.R: three ggplot2 wrappers on analysis summaries — `entropia_plot_temporal(x, date_var = NULL)` auto-detects the single POSIXct/Date bucket column (or takes `date_var` explicitly) and plots `n` as a line+point chart; `entropia_plot_entities(x, top = 10, fill = "entity_type")` plots the top `top` values as a horizontal bar chart with `value` reordered by count; `entropia_plot_coverage(x, metric = NULL)` plots `pct` by `group` (fill = group, percent y axis) faceted by `metric` when several metrics are present. Each function guards `requireNamespace("ggplot2")` first and aborts with `entropia_error_missing_dependency` when absent; all input validation carries `entropia_error_invalid_argument` (reusing `ent_require_tibble`/`ent_require_analysis_cols`/`ent_select_cols` from the analysis layer); every function returns a bare `ggplot` object, fully `+`-extensible. New error class `entropia_error_missing_dependency` also covers Task 25's arrow guard)
- [x] write tests: each returns a `ggplot` object (class assertion); data mapping correct; vdiffr snapshot on fixture data (tests/testthat/test-plot.R, 54 assertions + 3 vdiffr snapshots: class + `rlang::as_label` mapping assertions (x/y/fill) per plot, date-column auto-detection and explicit `date_var`, `top` slicing with count-reorder ordering, custom `fill` (bare and string), facet presence (`FacetWrap` multi-metric vs `FacetNull` single-metric), `ggplot_build` render proof, `+ labs(...)` extensibility, and the full invalid-argument error matrix (missing/ambiguous columns, `top`/`metric`/`fill` validation, empty input, non-data.frame, lazy table); vdiffr `expect_doppelganger` snapshots guarded by `skip_if_not_installed("vdiffr")` so the suite stays green without it (auto-generated on the first CI run where vdiffr is installed))
- [x] run tests — must pass before Task 24 (full suite green, 0 fail, 1 vdiffr skip; lint matches the established profile — only the known cross-file `object_usage` local artifact (R/plot.R 17, test-plot.R 10; the accepted R/analysis.R baseline is 30 of the same kind); `devtools::check()` 0 errors / 0 warnings / 2 known baseline notes — env clock "unable to verify current time" + unused Imports lifecycle/stringr reserved for Tasks 26-28; examples OK)
- **Acceptance:** plots render and remain user-extensible (`+ labs(...)`).
  - Verified 2026-08-06 with R 4.5.2 on the full fixture: all three plots render to a device (ggplot_build + print), mappings resolve as documented (temporal x=created_at/y=n; entities x=value/y=n/fill=entity_type; coverage x=group/y=pct/fill=group), `entropia_plot_coverage()` facets by metric (`FacetWrap`) while `metric = "ocr_coverage"` renders unfaceted (`FacetNull`); every plot accepts `+ labs(...)`; the single `empty_text` NA-`pct` row (total 0) is dropped by `geom_col` as documented.

### Phase 5 — Export & stubs (after 13; tasks 24–25 parallel)

### Task 24: entropia_export (csv/tsv/json/rds + streaming)
- [x] implement `entropia_export()` with csv/tsv/json/rds; deterministic ordering; chunked streaming for lazy inputs (`R/export.R`: `entropia_export(x, path, format = c("csv","tsv","json","rds","parquet","arrow"), chunk_size = 1000L)` — csv/tsv/json/rds always available plus parquet/arrow behind a `requireNamespace("arrow")` guard reusing the `entropia_error_missing_dependency` class (Task 23's); lazy `tbl_sql` inputs stream to csv/tsv via `DBI::dbSendQuery` + `fetch(n)` chunks (`ent_export_delimited_lazy`), never collecting the whole result; json/rds/parquet/arrow are whole-file formats and collect first; lazy exports are deterministically ordered — `ent_export_ordered()` arranges by the first column when the rendered SQL carries no `ORDER BY` (leaving explicit orderings such as `entropia_search`'s rank untouched, conservatively); delimited output writes with `qmethod = "double"` (RFC-4180) so fields holding both embedded quotes and newlines (e.g. extraction text) round-trip through base `read.csv` — the default `"escape"` method mangles them in R's own parser (discovered on the real DB); list-columns flatten to JSON strings and BLOB cells to space-joined hex bytes via `ent_prepare_delimited`; lazy-input connections validated with `ent_require_conn` for a clean closed-connection error)
- [x] write tests: round-trips (export → import → identical); streamed vs collected equivalence on larger fixture; format validation errors (`tests/testthat/test-export.R`, 47 assertions: csv/tsv/json/rds round-trips on materialised tibbles, default-format csv, RFC-4180 multiline+quotes round-trip, list/BLOB flattening, lazy csv ordered + matching collect, a 250-row table streamed with `chunk_size = 13` byte-identical to one materialised write of the same ordered data, two exports of the same query byte-identical, empty-query export writes only the header, corpus tsv streaming, json lazy parse-back, rds dataset round-trip with class + provenance intact, rds lazy = collect, parquet/arrow round-trips behind `skip_if_not_installed("arrow")`, and the full invalid-argument matrix for format/path/x/chunk_size plus closed-connection rejection)
- [x] run tests — must pass before Task 25 (full suite 1460 pass / 0 fail / 0 warn / 1 pre-existing vdiffr skip; lint matches the established profile — cross-file `object_usage` artifact + test SCREAMING_SNAKE constants only; `devtools::check()` 0 errors / 0 warnings / 2 known baseline notes; examples OK)
- **Acceptance:** exporting the corpus to CSV on the real DB streams without a full collect spike.
  - Verified 2026-08-06 with R 4.5.2 on a `VACUUM INTO` copy of the 29 MB reference DB (the Task 8 live-open segfault note still applies; sqlite3 CLI made the copy): `entropia_export(entropia_corpus(con), "corpus.csv", chunk_size = 500)` streams the full corpus in ~0.4 s; the CSV round-trips through `read.csv` back to exactly **2477 rows / 19 columns**, ordered by `item_id`, and is byte-identical to a single materialised write of the same ordered data; tsv/json/rds also export the full corpus (2477 rows) correctly.

### Task 25: parquet/arrow + write stubs
- [x] implement `format = "parquet"/"arrow"` behind `requireNamespace("arrow")` with clear missing-dep error (export side landed in Task 24's `ent_export_arrow`; the missing-dep guard was refactored to a testable `ent_arrow_available()` seam in `R/export.R` so the error path can be exercised deterministically)
- [x] implement v2 write stubs: `entropia_insert/update/upsert/delete` abort with `entropia_error_write_disabled` + documented v2 guidance; `entropia_connect(write = TRUE)` errors (new `R/write.R` with the four verbs on a shared `ent_write_disabled()` helper; roxygen docs carry the v2 contract signatures; `entropia_connect(write = TRUE)` already rejected at the door since Task 4)
- [x] write tests: parquet round-trip when arrow present (skip otherwise); every stub errors with correct class and actionable message (`tests/testthat/test-write.R`, 26 assertions: per-verb class, hard-fail even without a live connection, message guidance (read-only / v2 / `write = TRUE` / vignette), per-verb message text, `connect(write = TRUE)` class, exports present; `test-export.R` gains the arrow-absent mock test on the `ent_arrow_available` seam — parquet/arrow raise `entropia_error_missing_dependency` with the "arrow" message while csv still works)
- [x] run tests — must pass before Task 26 (full suite 1491 pass / 0 fail / 0 warn / 1 pre-existing vdiffr skip; lint-clean on `R/write.R` + `R/export.R`, tests match the established profile (SCREAMING_SNAKE constants only); `devtools::check()` 0 errors / 0 warnings / 1 known baseline note — unused Imports lifecycle/stringr reserved for Tasks 26-28)
- **Acceptance:** `R CMD check` passes with arrow installed AND without (Suggests discipline).
  - Verified 2026-08-06 with R 4.5.2: arrow 22.0.0 installed — parquet/arrow round-trip tests run green; the arrow-absent path is proven by the `ent_arrow_available` mock (correct `entropia_error_missing_dependency` class + message, csv unaffected); `devtools::check()` 0 errors / 0 warnings / 1 baseline note with arrow present, and the design keeps `arrow` in Suggests so an arrow-less check skips the round-trip via `skip_if_not_installed("arrow")` while the mock still exercises the error path. The write stubs abort with `entropia_error_write_disabled` and actionable v2 guidance; `entropia_connect(path, write = TRUE)` rejects before any open attempt.
...
- [x] add lifecycle badges (experimental on analysis layer); `lifecycle::deprecate_warn` scaffolding documented (all 6 analysis-layer functions carry `r lifecycle::badge("experimental")` in `R/analysis.R`; new internal `ent_deprecate()` wrapper around `lifecycle::deprecate_warn` in `R/utils.R` and the one-release-grace deprecation policy documented in the package help)
- [x] audit every exported error/warning for cli formatting + class + actionable message; snapshot tests for key messages (4 bare sites fixed: `R/collect.R` + `R/corpus.R` malformed-JSON `warning()` → `cli::cli_warn(..., class = "entropia_warn_malformed_json")`; `R/connect.R` + `R/schema.R` re-raising `stop(cond)` → `rlang::cnd_signal(cond)`; grep shows zero `stop(`/`warning(` left in `R/`)
- [x] write tests: message snapshots; badge presence in docs (`tests/testthat/test-lifecycle.R`, 35 assertions: badge present on the 6 analysis Rd files and absent from the stable accessor Rd files, `ent_deprecate` emits `lifecycle_warning_deprecated`, malformed-JSON warnings carry `entropia_warn_malformed_json` on both the collect path and `entropia_metadata`, source grep gate; snapshots recorded in `_snaps/lifecycle.md`)
- [x] run tests — must pass before Task 29 (full suite 1526 pass / 0 fail / 0 warn / 1 pre-existing vdiffr skip; lint-clean on all changed files; `devtools::check()` 0 errors / 0 warnings / 1 known baseline note — now only the unused-Imports `stringr` note, since `lifecycle` is consumed by `ent_deprecate()`; the remaining note is Task 30's to resolve or document in NEWS)
- **Acceptance:** no bare `stop()`/`warning()` without class+cli in exported code (`grep` gate in CI task).
  - Verified 2026-08-06 with R 4.5.2: `grep -E '\bstop\s*\(|\bwarning\s*\(' R/*.R` → 0 matches. Every remaining message site uses `ent_abort`/`cli::cli_warn`/`cli::cli_inform`/`rlang::cnd_signal` with a stable condition class; the `test-lifecycle.R` source gate enforces this on the source tree and skips for installed-package check runs (the CI grep job covers `R CMD check`). Environment note: `Rscript -f`/`-e` segfaults on this machine when RSQLite loads (Task 8 note) — all validation ran under `R.exe --vanilla -f script.R`; the check subprocess needed `RSTUDIO_PANDOC` for the vignette build step.
...
- [x] resolve every warning/note; document unavoidable notes in NEWS (the `stringr` unused-Imports note is resolved by dropping the genuinely unused import — marker stripping is SQL (recursive CTE) and search-term hygiene is `dbQuoteString`, so no string helper was ever needed; NEWS documents the removal)
- [x] performance pass: `EXPLAIN QUERY PLAN` on corpus/search/entity-join; fix any table scans on hot paths; add a smoke test asserting no full-scan on the corpus join (plans verified on the full fixture: unfiltered corpus joins collections (`sqlite_autoindex_collections_1` PK), assets (`idx_assets_item_sort`), extractions/transcriptions (`idx_extractions_asset_id_unique`/`idx_transcriptions_asset_id_unique`) — only the driving `items` table scans, which is unavoidable; a collection-filtered corpus drives into items via `idx_items_collection`; FTS search uses the virtual-table index + INTEGER PRIMARY KEY rowid/chunk_id lookups; entity-relations resolves items/collections by PK. No joined lookup table full-scans, so nothing needed fixing; the invariant is now enforced by `tests/testthat/test-performance.R`)
- [x] write tests: performance smoke test (EXPLAIN-based); full regression run (`tests/testthat/test-performance.R`, 13 assertions: corpus join index-invariant, filtered corpus via `idx_items_collection`, items/chunks FTS virtual-table + PK-lookup plans, entity-relations PK lookups; robust across RSQLite/SQLite versions because SQLite's planner assumes large tables without ANALYZE so the right index is always preferred)
- [x] run tests — must pass before Task 31 (full testthat suite 1539 pass / 0 fail / 0 warn / 1 pre-existing vdiffr skip; lint 0 findings; styler 0 files changed; `devtools::check()` → **Status: OK — 0 errors / 0 warnings / 0 notes** with R 4.5.2, `RSTUDIO_PANDOC` set for the vignette rebuild)

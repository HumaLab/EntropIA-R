...
Thanks for helping out! This document covers the development workflow for the
package itself. If you are here to *use* entropiaR, see the
[README](README.en.md).
...
- **Windows**: R is typically *not* on `PATH`. Either add
  `C:\Program Files\R\R-4.5.2\bin` to your `PATH`, or invoke R by full path,
  e.g. `"C:\Program Files\R\R-4.5.2\bin\Rscript.exe" -e "..."`.
...
- Each test receives a **temp copy** (via `ent_fixture()`), so tests never
  mutate the originals.
- Never use `data-test/entropia.sqlite` as a test fixture (reference only).

## Conventions

- **Commits**: Conventional Commits, e.g. `feat:`, `fix:`, `test:`, `docs:`.
  No AI attribution lines.
- **Errors**: all conditions go through `ent_abort()` with a stable
  `entropia_error_<kind>` class — no bare `stop()`/`warning()` in `R/`.

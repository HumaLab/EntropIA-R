...
#' @param min_confidence Optional entity confidence threshold in \[0, 1\].
...
entropia_overview <- function(con, collection_ids = NULL, asset_types = NULL,
  ...
  text_source <- ent_validate_choice(text_source, c("auto", "extraction", "transcription"), "text_source")
  ...
  study <- ent_study_query(con, collection_ids = collection_ids,
  ...
  prefix <- paste0("WITH s AS (", query, "), si AS (SELECT DISTINCT item_id, collection_id FROM s WHERE item_id IS NOT NULL), sa AS (SELECT DISTINCT asset_id, item_id, collection_id, asset_type FROM s WHERE asset_id IS NOT NULL) ")
  get <- function(sql) tibble::as_tibble(DBI::dbGetQuery(con, paste0(prefix, sql)))
  has <- function(tab, cols) DBI::dbExistsTable(con, tab) && all(cols %in% DBI::dbListFields(con, tab))
  ...
  collections <- get(paste0("SELECT c.id AS collection_id, c.name AS collection_name, COALESCE(z.n_items,0) AS n_items, COALESCE(z.n_assets,0) AS n_assets, COALESCE(z.n,0) AS n FROM collections c LEFT JOIN (SELECT collection_id, COUNT(DISTINCT item_id) AS n_items, COUNT(DISTINCT asset_id) AS n_assets, COUNT(*) AS n FROM s GROUP BY collection_id) z ON c.id=z.collection_id WHERE ", collection_where, " ORDER BY c.id"))
  page_expr <- if (has("assets", "parent_asset_id")) {
    "(SELECT COUNT(*) FROM sa JOIN assets a ON a.id=sa.asset_id WHERE a.parent_asset_id IS NOT NULL)"
  } else "0"
  counts <- get(paste0("SELECT 'items' AS metric, 'item' AS unit, COUNT(*) AS n FROM si UNION ALL SELECT 'assets','asset',COUNT(*) FROM sa UNION ALL SELECT 'collections','collection',COUNT(DISTINCT collection_id) FROM si UNION ALL SELECT 'items_without_assets','item',COUNT(*) FROM si WHERE NOT EXISTS (SELECT 1 FROM sa WHERE sa.item_id=si.item_id) UNION ALL SELECT 'pages','asset',", page_expr))
  ...
  bucket <- paste0("CASE WHEN typeof(", stamp, ") IN ('integer','real') THEN datetime(", stamp, "/1000.0,'unixepoch','start of month') END")
  temporal <- get(paste0("SELECT ", bucket, " AS date,COUNT(*) AS n FROM s GROUP BY ", bucket, " ORDER BY date"))
  exclusions <- get(paste0("SELECT COALESCE(SUM(", stamp, " IS NULL),0) AS missing, COALESCE(SUM(", stamp, " IS NOT NULL AND (", bucket, ") IS NULL),0) AS invalid FROM s"))
  ...
  entities <- tibble::tibble(entity_type = character(), value = character(), n = integer(), n_items = integer(), total = numeric(), pct = numeric())
  if (has("entities", c("item_id", "entity_type", "value"))) {
    conditions <- "EXISTS (SELECT 1 FROM si WHERE si.item_id=e.item_id)"
    if (has("entities", "asset_id")) conditions <- paste0(conditions, " AND (e.asset_id IS NULL OR EXISTS (SELECT 1 FROM sa WHERE sa.asset_id=e.asset_id AND sa.item_id=e.item_id))")
    filters <- list(source = entity_source, model_name = model_name)
    for (nm in names(filters)) {
      val <- filters[[nm]]
      if (!is.null(val)) {
        if (!has("entities", nm)) ent_eda_arg("The entities table lacks the requested filter column {.val {nm}}.")
        conditions <- paste(conditions, "AND", in_sql(paste0("e.", qi(nm)), val))
      }
    }
    if (is.null(entity_source) && has("entities", "source")) conditions <- paste0(conditions, " AND (e.source IS NULL OR e.source <> 'manual_deleted')")
    if (!is.null(min_confidence)) {
      if (!has("entities", "confidence")) ent_eda_arg("The entities table lacks confidence; pass min_confidence = NULL.")
      conditions <- paste0(conditions, " AND e.confidence >= ", as.character(DBI::dbQuoteLiteral(con, min_confidence)))
    }
    entities <- get(paste0("SELECT e.entity_type,e.value,COUNT(*) AS n,COUNT(DISTINCT e.item_id) AS n_items FROM entities e WHERE ", conditions, " GROUP BY e.entity_type,e.value ORDER BY e.entity_type,n DESC,e.value"))
    entities$total <- rep(total, nrow(entities))
    entities$pct <- if (total == 0) rep(NA_real_, nrow(entities)) else entities$n_items / total
  }
  topics <- tibble::tibble(name = character(), n = integer(), n_items = integer(), total = numeric(), pct = numeric())
  if (has("topics", c("id", "name")) && has("item_topics", c("item_id", "topic_id"))) {
    topics <- get("SELECT t.name,COUNT(*) AS n,COUNT(DISTINCT it.item_id) AS n_items FROM item_topics it JOIN topics t ON t.id=it.topic_id WHERE EXISTS (SELECT 1 FROM si WHERE si.item_id=it.item_id) GROUP BY t.name ORDER BY n DESC,t.name")
  ...
  selection <- list(collection_ids = collection_ids, asset_types = asset_types,
  ...
  provenance <- list(schema_version = ent_attr(con, "schema_version"),
  ...
  list(counts = counts, collections = collections, asset_types = types,
...
ent_eda_quality <- function(con, get, has, text_source, collections) {
  out <- tibble::tibble(metric = character(), group_id = character(), group = character(),
    unit = character(), n = numeric(), total = numeric(), pct = numeric(), status = character())
  add <- function(metric, x, unit, status = NULL) {
    ...
    out <<- dplyr::bind_rows(out, tibble::tibble(metric = metric,
      group_id = as.character(x$group_id), group = as.character(x$group), unit = unit,
      n = as.numeric(x$n), total = as.numeric(x$total),
  ...
  present <- paste0("(", meta, " IS NOT NULL AND trim(", meta, ", char(9)||char(10)||char(13)||' ') <> '')")
  mq <- get(paste0("SELECT si.collection_id AS group_id, COUNT(*) AS total, SUM(", present, ") AS n, SUM(CASE WHEN ", present, " THEN json_valid(", meta, ") ELSE 0 END) AS valid FROM si JOIN items i ON i.id=si.item_id GROUP BY si.collection_id"))
  m <- dplyr::left_join(dplyr::transmute(collections, group_id = as.character(.data$collection_id), group = .data$collection_name),
    dplyr::mutate(mq, group_id = as.character(.data$group_id)), by = "group_id")
  for (nm in c("total", "n", "valid")) m[[nm]][is.na(m[[nm]])] <- 0
  add("metadata_coverage", m, "item", if (!meta_available) rep("not_applicable", nrow(m)) else ifelse(m$total == 0, "no_data", ifelse(m$n == 0, "empty", "ok")))
  valid <- m
  valid$total <- m$n
  valid$n <- m$valid
  add("metadata_validity", valid, "item", if (!meta_available) rep("not_applicable", nrow(m)) else ifelse(m$total == 0, "no_data", ifelse(m$n == 0, "empty", ifelse(m$valid < m$n, "invalid", "ok"))))
  ...
  layer <- function(tab, flag, useful, available) {
    if (!available) return(paste0("SELECT NULL AS asset_id,0 AS ", flag, ",0 AS ", useful, " WHERE 0"))
    paste0("SELECT asset_id,1 AS ", flag, ",MAX(CASE WHEN text_content IS NOT NULL AND trim(text_content,char(9)||char(10)||char(13)||' ') <> '' THEN 1 ELSE 0 END) AS ", useful, " FROM ", tab, " GROUP BY asset_id")
  }
  base <- paste0("SELECT sa.*,COALESCE(e.ep,0) AS ep,COALESCE(e.eu,0) AS eu,COALESCE(t.tp,0) AS tp,COALESCE(t.tu,0) AS tu FROM sa LEFT JOIN (", layer("extractions", "ep", "eu", ep), ") e ON e.asset_id=sa.asset_id LEFT JOIN (", layer("transcriptions", "tp", "tu", tp), ") t ON t.asset_id=sa.asset_id")
  selected_present <- switch(text_source, auto = "(ep OR tp)", extraction = "ep", transcription = "tp")
  selected_useful <- switch(text_source, auto = "CASE WHEN ep=1 THEN eu ELSE tu END", extraction = "eu", transcription = "tu")
  eligible <- switch(text_source, auto = "asset_type IN ('pdf','image','audio','video')", extraction = "asset_type IN ('pdf','image')", transcription = "asset_type IN ('audio','video')")
  aggregate <- function(n, total = "1") get(paste0("SELECT asset_type AS group_id,asset_type AS \"group\",COALESCE(SUM(", n, "),0) AS n,COALESCE(SUM(", total, "),0) AS total FROM (", base, ") b GROUP BY asset_type ORDER BY asset_type"))
  add("ocr_coverage", aggregate("eu"), "asset", if (!ep) rep("not_applicable", nrow(aggregate("eu"))) else NULL)
  add("transcription_presence", aggregate("tp"), "asset", if (!tp) rep("not_applicable", nrow(aggregate("tp"))) else NULL)
  ...
  available <- switch(text_source, auto = ep || tp, extraction = ep, transcription = tp)
  status <- if (!available) rep("not_applicable", nrow(usable)) else ifelse(usable$total == 0, "not_applicable", ifelse(usable$n == 0, "empty", "ok"))
  add("selected_text_usable", usable, "asset", status)
  empty <- aggregate(paste0("(", eligible, ") AND ", selected_present, " AND NOT (", selected_useful, ")"), paste0("(", eligible, ") AND ", selected_present))
  add("selected_text_empty", empty, "asset", ifelse(!available | usable$total == 0, "not_applicable", ifelse(empty$total == 0, "no_data", ifelse(empty$n > 0, "empty", "ok"))))
  dplyr::arrange(out, .data$metric, .data$group_id)
}
...
entropia_profile <- function(x, columns = NULL, by = NULL, sample_n = NULL, seed = 1L) {
  ...
  reserved <- c("variable", "statistic", "value", "n", "total", "pct", "status", "rule", "n_groups", "n_rows")
  if (length(intersect(groups, reserved))) ent_eda_arg("Rename grouping columns that conflict with profile result columns: {.val {intersect(groups, reserved)}}.")
  valid_integer <- function(z) is.numeric(z) && length(z) == 1L && is.finite(z) && z == floor(z) && abs(z) <= .Machine$integer.max
  if (!is.null(sample_n) && (!valid_integer(sample_n) || sample_n < 0)) ent_eda_arg("{.arg sample_n} must be NULL or a nonnegative integer.")
  ...
  if (!is.null(sample_n)) {
    ...
    saved <- if (existed) get(".Random.seed", envir = .GlobalEnv, inherits = FALSE) else NULL
    on.exit({
      if (existed) assign(".Random.seed", saved, envir = .GlobalEnv)
  ...
  missing_value <- function(z) {
    if (is.list(z)) vapply(z, function(v) is.null(v) || (length(v) == 1L && is.atomic(v) && is.na(v)), logical(1))
    else is.na(z)
  }
  structure <- dplyr::bind_rows(lapply(cols, function(nm) {
    ...
    tibble::tibble(variable = nm, class = paste(class(z), collapse = "/"),
  ...
  if (!length(cols)) structure <- tibble::tibble(variable = character(), class = character(), type = character(), n = integer(), n_missing = integer(), min = character(), max = character(), contract = character())
  group_template <- x[0, groups, drop = FALSE]
  missing <- dplyr::bind_cols(group_template, tibble::tibble(variable = character(), n = integer(), total = integer(), pct = numeric(), status = character()))
  numeric <- dplyr::bind_cols(group_template, tibble::tibble(variable = character(), statistic = character(), value = numeric()))
  categorical <- dplyr::bind_cols(group_template, tibble::tibble(variable = character(), value = character(), n = integer(), total = integer(), pct = numeric()))
  duplicates <- dplyr::bind_cols(group_template, tibble::tibble(rule = character(), n = integer(), n_groups = integer(), n_rows = integer(), total = integer()))
  if (length(groups)) {
  ...
  for (i in seq_along(parts)) {
    ...
    for (nm in variables) {
      ...
      missing <- dplyr::bind_rows(missing, attach_key(tibble::tibble(variable = nm,
        n = sum(miss), total = length(z), pct = if (length(z)) mean(miss) else NA_real_,
        status = if (!length(z)) "no_data" else if (invalid) "invalid" else if (all(miss)) "empty" else "ok")))
      if (number) {
        v <- z[is.finite(z)]
        q <- if (length(v)) as.numeric(stats::quantile(v, c(0.25, 0.5, 0.75), names = FALSE)) else rep(NA_real_, 3L)
        spread <- q[3L] - q[1L]
        values <- c(length(v), if (length(v)) mean(v) else NA_real_,
        ...
        numeric <- dplyr::bind_rows(numeric, attach_key(tibble::tibble(variable = nm,
          statistic = c("n", "mean", "sd", "min", "q25", "median", "q75", "max", "IQR", "outliers"), value = as.numeric(values))))
        ...
        categorical <- dplyr::bind_rows(categorical, attach_key(tibble::tibble(variable = rep(nm, nrow(freq)), value = freq$value, n = freq$n,
          total = rep(length(z), nrow(freq)), pct = freq$n / length(z))))
      }
    }
    rules <- list(exact_rows = cols, exact_keys = cols[grepl("(^id$|_id$)", cols, ignore.case = TRUE)])
    for (rule in names(rules)) {
      ...
      duplicates <- dplyr::bind_rows(duplicates, attach_key(tibble::tibble(rule = rule,
        n = sum(later), n_groups = sum(involved & !later), n_rows = sum(involved), total = nrow(part))))
    }
  }
  list(structure = structure, missing = missing, numeric = numeric,
    categorical = categorical, duplicates = duplicates,
    sampling = list(original_n = original_n, n = nrow(x), sample_n = sample_n,

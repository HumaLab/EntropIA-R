...
ent_plot_empty <- function(title, message = "Sin datos para esta selección") {
  ggplot2::ggplot(data.frame(x = 0, y = 0), ggplot2::aes(x = .data$x, y = .data$y)) +
    ggplot2::geom_text(label = message) + ggplot2::theme_void() +
    ggplot2::labs(title = title)
}

ent_plot_note <- function(p, n) {
  if (n > 0L) p <- p + ggplot2::labs(caption = paste(n, "observaciones sin valor válido; no representadas"))
  p
}
...
ent_plot_group <- function(x, group, exclude = character()) {
  if (!is.null(group)) return(ent_plot_identity(x, group))
  ...
  if (length(candidates) > 1L) {
    ent_plot_error(paste0("Ambiguous grouping columns: ", paste(candidates, collapse = ", "),
...
ent_plot_key <- function(x, cols) {
  if (!length(cols)) return(rep.int(1L, nrow(x)))
  dplyr::group_indices(dplyr::group_by(x, dplyr::across(dplyr::all_of(unique(cols)))))
}

ent_plot_facets <- function(p, facet) {
  if (!is.null(facet)) p <- p + ggplot2::facet_wrap(ggplot2::vars(!!rlang::sym(facet)), scales = "free_x")
  p
}
...
ent_plot_bars <- function(x, label, value, title, fill = NULL, facet = NULL) {
  if (!nrow(x)) return(ent_plot_empty(title))
  ...
  p <- p + ggplot2::geom_col(data = x[ok, , drop = FALSE]) +
    ggplot2::scale_x_discrete(labels = stats::setNames(x$.ent_label, as.character(x$.ent_bar)), drop = FALSE) +
  ...
  if (any(!ok)) {
    ...
    p <- p + ggplot2::geom_text(data = bad,
...
ent_plot_top <- function(x, top, value, group, facet, top_by) {
  ...
  idx <- unlist(lapply(split(seq_len(nrow(x)), keys), function(i) {
    head(i[order(-rank_value[i], i, na.last = TRUE)], as.integer(top))
  }), use.names = FALSE)
  x[idx, , drop = FALSE]
}
...
#' @export
entropia_plot_distribution <- function(x, var, by = NULL, type = "histogram", bins = 30, log = FALSE) {
  x <- ent_plot_data(x)
  ...
  if (!is.logical(log) || length(log) != 1L || is.na(log)) ent_plot_error("log must be TRUE or FALSE.")
  if (!is.numeric(bins) || length(bins) != 1L || !is.finite(bins) ||
      bins < 1 || bins > .Machine$integer.max || bins != floor(bins)) ent_plot_error("bins must be a positive integer.")
  ok <- is.finite(x[[var]]) & (!log | x[[var]] > 0)
  omitted <- sum(!ok)
  x <- x[ok, , drop = FALSE]
  if (!nrow(x)) return(ent_plot_empty("Distribución", "Sin observaciones válidas para esta escala"))
  if (type == "boxplot") {
    ...
    p <- ggplot2::ggplot(x, ggplot2::aes(x = .data$.ent_group, y = !!rlang::sym(var))) +
      ggplot2::geom_boxplot() + ggplot2::labs(x = by, y = var)
    if (log) p <- p + ggplot2::scale_y_log10()
  } else {
    p <- ggplot2::ggplot(x, ggplot2::aes(x = !!rlang::sym(var)))
    if (!is.null(by)) p <- p + ggplot2::aes(colour = factor(!!rlang::sym(by)), fill = factor(!!rlang::sym(by)))
    if (type == "histogram") {
      p <- p + ggplot2::geom_histogram(bins = as.integer(bins), position = "identity", alpha = 0.5) + ggplot2::labs(y = "Frecuencia")
    } else {
      p <- p + ggplot2::stat_ecdf(geom = "step", pad = FALSE) + ggplot2::labs(y = "Proporción acumulada")
    }
...
entropia_plot_missing <- function(x) {
  ...
  ent_plot_bars(x, ".ent_display", "pct", "Valores ausentes") +
    ggplot2::scale_y_continuous(labels = ent_pct_labels) + ggplot2::labs(y = "Proporción ausente")
}
...
entropia_plot_scatter <- function(x, x_var, y_var, by = NULL) {
  ...
  omitted <- sum(!ok)
  x <- x[ok, , drop = FALSE]
  if (!nrow(x)) return(ent_plot_empty("Dispersión", "Sin pares de valores válidos"))
  p <- ggplot2::ggplot(x, ggplot2::aes(x = !!rlang::sym(xc), y = !!rlang::sym(yc)))
  if (!is.null(by)) p <- p + ggplot2::aes(colour = factor(!!rlang::sym(by)))
  ent_plot_note(p + ggplot2::geom_point() + ggplot2::labs(title = "Dispersión", colour = by), omitted)
}
...
entropia_plot_correlation <- function(x, columns) {
  ...
  cols <- cols[vapply(x[cols], function(z) is.numeric(z) && !inherits(z, c("Date", "POSIXt", "difftime")), logical(1))]
  if (length(cols) < 2L || !nrow(x)) return(ent_plot_empty("Correlaciones", "Se necesitan dos mediciones numéricas y observaciones"))
  ...
  d$label <- ifelse(is.finite(d$correlation), sprintf("%.2f", d$correlation), "Sin datos\no constante")
  ggplot2::ggplot(d, ggplot2::aes(x = .data$variable_x, y = .data$variable_y, fill = .data$correlation)) +
    ggplot2::geom_tile() + ggplot2::geom_text(ggplot2::aes(label = .data$label)) +
...
entropia_plot_temporal <- function(x, date_var = NULL, group = NULL, facet = NULL, value = "n") {
  ...
  if (is.null(date_col)) {
    ...
    if (length(date_col) != 1L) ent_plot_error("Select date_var explicitly: exactly one Date or POSIXct column is required.")
  }
  if (!date_col %in% ent_date_cols(x)) ent_plot_error("date_var must select a Date or POSIXct column.")
  value <- ent_plot_col(x, rlang::enquo(value), "value")
  ent_plot_numeric(x, value)
  facet <- ent_plot_identity(x, ent_plot_col(x, rlang::enquo(facet), "facet"))
  group <- ent_plot_group(x, ent_plot_col(x, rlang::enquo(group), "group"), c(date_col, value, facet))
  ok <- is.finite(as.numeric(x[[date_col]])) & is.finite(x[[value]])
  omitted <- sum(!ok)
  x <- x[ok, , drop = FALSE]
  if (!nrow(x)) return(ent_plot_empty("Evolución temporal", "Sin fechas y valores válidos"))
  keys <- unique(c(date_col, group, facet))
  if (anyDuplicated(x[, keys, drop = FALSE])) {
    ent_plot_error("Multiple rows share a date and series. Select group/facet to distinguish them, or aggregate explicitly before plotting.")
  }
  ...
  p <- ggplot2::ggplot(x, ggplot2::aes(x = !!rlang::sym(date_col),
...
#' }
entropia_plot_entities <- function(x, top = 10, fill = "entity_type", group = NULL, facet = NULL, top_by = "global", value = "n") {
  x <- ent_plot_data(x, "value")
  if (nrow(x) == 0L) return(ent_plot_empty("Entidades principales"))
  fill <- ent_plot_identity(x, ent_plot_col(x, rlang::enquo(fill), "fill"))
  value <- ent_plot_col(x, rlang::enquo(value), "value")
  facet <- ent_plot_identity(x, ent_plot_col(x, rlang::enquo(facet), "facet"))
  group <- ent_plot_group(x, ent_plot_col(x, rlang::enquo(group), "group"),
...
entropia_plot_coverage <- function(x, metric = NULL) {
  ...
  if (!nrow(x)) return(ent_plot_empty("Cobertura del corpus"))
